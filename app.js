...
function reconciliation(data) {
  return `<div class="recon-card"><div class="recon-side"><span>AGRÉGAT BATCH</span><strong>${integer.format(data.batch_units)}</strong></div><div class="recon-equals">${icon("check")}</div><div class="recon-side"><span>ÉVÉNEMENTS KINESIS</span><strong>${integer.format(data.stream_units)}</strong></div></div><div class="recon-foot"><span>Écart comptable : <b>${data.delta}</b></span><span class="badge pass">${icon("check")} ${data.status}</span></div>`;
}
...
function renderOverview() {
  ...
  root.innerHTML = hero("RETAIL CORE · SINGLE SOURCE OF TRUTH", "Le retail en un seul regard", "Ventes synthétiques omnicanales, disponibilité stock et fiabilité du pipeline sur la sélection active.", `<button type="button" class="subtle-button" data-view-jump="pipeline">Voir l’architecture</button><button type="button" class="subtle-button accent" data-open-sim>Scénario Black Friday</button>`)
    ...
    + `<div class="dashboard-grid">${panel("Performance commerciale", "Chiffre d’affaires quotidien · filtre actif", lineChart(d.series), 8, `<div class="legend"><span><i></i>CA sélectionné</span></div>`)}${panel("Mix des canaux", "Contribution dans la même sélection", donut(d.channel_mix), 4)}${panel("Catégories motrices", "Top 7 catégories par chiffre d’affaires", categoryBars(d.categories), 5)}${panel("Réconciliation batch / streaming", `Même périmètre · ${d.meta.scope}`, reconciliation(d.reconciliation), 7, `<span class="badge ${d.reconciliation.status === "PASS" ? "pass" : "critical"}">${d.reconciliation.delta === 0 ? "ZÉRO ÉCART" : `ÉCART ${d.reconciliation.delta}`}</span>`)}</div>`;
  bindChartTooltips();
  bindInlineActions();
}
...
function renderRealtime() {
  ...
  root.innerHTML = hero("STREAMING RETAIL · REPLAY SYNTHÉTIQUE", "Le parcours événementiel sous contrôle", "Consultations, paniers et achats sont filtrés, validés puis rapprochés du batch sur le même périmètre.", `<button type="button" class="subtle-button accent" data-open-sim>Estimer un trafic ×5</button>`)
    ...
    + `<div class="dashboard-grid">${panel("Replay des événements", "12 événements les plus récents de la sélection", feed, 5, `<span class="badge healthy">● REPLAY</span>`)}${panel("Valeur captée", "Ventes quotidiennes corrélées aux achats", lineChart(d.series), 7, `<span>p95 ${integer.format(d.kpis.latency_p95_ms)} ms</span>`)}${panel("Garantie d’exactitude", "Idempotence métier et rapprochement sur le même filtre", reconciliation(d.reconciliation), 6)}${panel("Résilience : implémenté et cible", "Distinction explicite entre le démonstrateur et la production", `<div class="check-grid"><div class="check-item"><div class="check-mark">${icon("check")}</div><div><strong>event_id unique</strong><span>Testé localement</span></div></div><div class="check-item"><div class="check-mark">${icon("check")}</div><div><strong>Partition customer_id</strong><span>Testée localement</span></div></div><div class="check-item target"><div class="check-mark">${icon("arrow")}</div><div><strong>Dead-letter queue</strong><span>Cible production</span></div></div><div class="check-item target"><div class="check-mark">${icon("arrow")}</div><div><strong>CloudWatch alarms</strong><span>Cible production</span></div></div></div>`, 6)}</div>`;
...
function renderPipeline() {
  const d = state.data, run = d.pipeline_run;
  ...
  const rowsProcessed = Number(rowCounts.orders || 0) + Number(rowCounts.events || 0);
  root.innerHTML = hero("PIPELINE LOCAL · ARCHITECTURE CLOUD CIBLE", "Une chaîne exécutable, testée et transposable", "Les mesures ci-dessous viennent du run Python local ; le schéma présente séparément les briques cloud visées.", `<button type="button" class="subtle-button accent" data-open-sim>Estimer la capacité</button>`)
    + `<div class="kpi-grid">${kpi("Run local", run.status, "dernier pipeline", "var(--lime)", "pipeline", `${run.phases.length} phases mesurées`)}${kpi("Durée réelle", `${Number(run.duration_ms).toFixed(1)} ms`, "machine locale", "var(--cyan)", "stream", "Télémétrie du run")}${kpi("Contrats qualité", `${d.quality.passed} / ${d.quality.total}`, "publishing gate", "var(--violet)", "shield", d.quality.status)}${kpi("Lignes traitées", integer.format(rowsProcessed), "ventes + événements", "var(--coral)", "bolt", "Jeu synthétique")}</div>`
    + `<div class="dashboard-grid">${panel("Architecture cloud cible", "Frontières de service prévues pour une industrialisation", flow, 12, `<span class="badge target">7 BRIQUES CIBLES</span>`)}${panel("Dernière exécution locale", "Durées réellement mesurées, sans prétendre exécuter Airflow ou AWS", execution, 7, `<span class="badge pass">${run.status}</span>`)}${panel("Preuves d’industrialisation", "Ce qui existe réellement dans le dépôt", `<div class="check-grid"><div class="check-item"><div class="check-mark">${icon("check")}</div><div><strong>Tests Python et dbt</strong><span>Publishing gate</span></div></div><div class="check-item"><div class="check-mark">${icon("check")}</div><div><strong>Docker healthcheck</strong><span>Exécution reproductible</span></div></div><div class="check-item"><div class="check-mark">${icon("check")}</div><div><strong>Clés idempotentes</strong><span>order_id et event_id</span></div></div><div class="check-item"><div class="check-mark">${icon("check")}</div><div><strong>DAG Airflow fourni</strong><span>Cible d’orchestration</span></div></div></div>`, 5)}</div>`;
  bindInlineActions();
}
...
function renderQuality() {
  ...
  root.innerHTML = hero("DATA CONTRACTS · TRUST BY DESIGN", "Des indicateurs auxquels le métier peut croire", "Le publishing gate bloque l’exposition si un contrat ou la réconciliation du périmètre sélectionné échoue.", `<a class="subtle-button" href="/api/quality-report" download="retail-core-quality-report.json">Télécharger le rapport JSON</a>`)
    + `<div class="kpi-grid">${kpi("Score qualité", `${q.score}%`, `${q.passed}/${q.total} contrôles`, "var(--lime)", "shield", publishable ? "Publishing gate ouvert" : "Publishing gate fermé")}${kpi("Fraîcheur", `${q.freshness_minutes} min`, "dernier événement généré", "var(--cyan)", "stream", "Seuil < 24 h")}${kpi("Écart comptable", `${d.reconciliation.delta}`, d.meta.scope, "var(--coral)", "cost", d.reconciliation.status)}${kpi("SCD Type 2", integer.format(d.price_scd.length), "versions chargées", "var(--violet)", "pipeline", `${integer.format(d.inventory.length)} produits × 2 versions`)}</div>`
